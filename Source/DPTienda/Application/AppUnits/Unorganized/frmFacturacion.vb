...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        ExplorerBarGroup4.Text = "Datos del Artículo"
        ...
        Select Case e.Command.Key
            ...
            Case "menuFacturarDev"
                ...
                If CDbl(strResult) > Int32.MaxValue Then
                    MsgBox("Numero de vale incorrecto, verifique sus datos", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, " Facturación")
                    Exit Sub
                End If
    ...
    Private Sub ebCodVendedor_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles ebCodVendedor.Validating
        ...
        If ebCodVendedor.Text.Trim <> "" AndAlso ebCodVendedor.Text.Trim <> oFactura.CodVendedor.Trim Then
            If IsNumeric(ebCodVendedor.Text.Trim()) = False Then
                MessageBox.Show("Código de vendedor debe ser numerico", "Dportenis PRO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Exit Sub
            End If
            oVendedor.ClearFields()
            ...
            If cantTotal > 1 Then
                ...
                For j As Integer = 1 To dtDetalleTemp.Rows.Count
                    ...
                    If dtView.Count > 0 Then
                        ...
                        End If
                        '------------------------------------------------------------------------------------------------------------------------
                        'RGERMAIN 07/05/2013: Se modificó por solicitud de Jose Oscar Sanchez las reglas del 2 x 1 ½, pero se dejó configurable 
                    ...
                    Case 0  'Columna CODIGO
                        ...
                        Else
                            ...
                            Else        '--NO ES UN CODIGO UPC
                                ...
                                '-------------------------------------------------------------------------------------------------------------
                                'FLIZARRAGA 12/07/2016: Validamos si existe el articulo por el código proveedor
                                '-------------------------------------------------------------------------------------------------------------
...
AplicaDescuentosDips:
    ...
    Private Function AplicaPromocionesVigentes(ByVal Porc As Decimal, ByRef oCupon As CuponDescuento, ByVal bAceptaPromo As Boolean) As Boolean
        ...
        dsTemp = AgruparCodigos(dsTemp).Copy
        '--------------------------------------------------------------------------------------------------------------------------------------
        'RGERMAIN 14.11.2014: Solo si no está la configuracion del Cross Selling activa actualizamos los descuentos aplicados en el detalle de
...
Inicio:
        ...
        If oRow >= 0 Then
            ...
            If IsDBNull(dsDetalle.Tables(0).Rows(oRow)) = False Then
                ...
                '        GoTo Inicio
                '    Else
                '        MessageBox.Show("Para el artículo " & Me.ebCodigoArticulo.Text.Trim & " no hay precio alimentado en SAP." & vbCrLf & "Favor de llamar a Soporte Tecnico.", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                '        GoTo Final
                '    End If
                'End If
    ...
    Private Function LoadSearchArticulo() As String
        ...
        If (oOpenRecordDlg.DialogResult = DialogResult.OK) Then
            ...
            If oOpenRecordDlg.pbCodigo = String.Empty Then
                ...
                Me.CodArticulo = CStr(oOpenRecordDlg.Record.Item("Código"))
                '------------------------------------------------------------------------------------------
    ...
    Private Sub CargarCodArticulo(ByVal indice As Integer)
        ...
        If ArtExistencia.Count > 0 Then
            ...
            If oFactura.FacturaArticuloExistencia > 0 Then
                ...
                If CodeRow > 0 Then
                    ...
                    If (grdDetalle(CodeRow - 1, 2) + 1) > ebExistencias.Value Then
                        ...
                        MsgBox("No se cuenta con existencia del artículo con esa talla", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, " Facturación")
            ...
            Else
                ...
                grdDetalle(grdDetalle.Row, 1) = ""
                grdDetalle(grdDetalle.Row, 2) = 0
                MessageBox.Show("No hay existencias del código", "Dportenis PRO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            End If
        Else
            grdDetalle(grdDetalle.Row, 0) = ""
            grdDetalle(grdDetalle.Row, 1) = ""
            grdDetalle(grdDetalle.Row, 2) = 0
            MessageBox.Show("No hay existencia del código", "Dportenis PRO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        End If
    End Sub
    ...
    Private Sub CargarUPC(ByVal RowIndex As Integer, ByVal ColIndex As Integer)
        ...
        Else
            ...
            If oCatalogoUPCMgr.IsSkuOrMaterial(grdDetalle(RowIndex, 0)) = "S" Then
                ...
                dsUPC = Nothing
            Else
                MessageBox.Show("No existe el código UPC", "Dportenis PRO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        ...
        If CdpVale.EEXIST = "S" Then
            ...
            If CdpVale.ESTATU <> "A" Then
                If CdpVale.ESTATU = "E" Then
                    status = "El Dpvale está Expirado"
                    Return status
                Else
...
#Region "Corrección ventas incompletas"
    ...
    Private Sub FormaPago()
        ...
        '-----------------------------------------------------------------------------------------------------------------------------
        ' MLVARGAS 06/07/2022: No se validaran los productos en aclaración a petición de Francisco Rojo
        '-----------------------------------------------------------------------------------------------------------------------------
