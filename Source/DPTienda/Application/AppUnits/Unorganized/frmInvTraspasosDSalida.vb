...
Public Class frmInvTraspasosDSalida
...
#Region " Código generado por el Diseñador de Windows Forms "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(frmInvTraspasosDSalida))
        ...
        Me.Name = "frmInvTraspasosDSalida"
...
#Region "Variables Miembros"
    ...
    Private oAlmacenOrigen As Almacen
...
#Region "Métodos Privados"
    ...
    Private Sub Sm_TxtLimpiar()
        ...
        oAlmacenOrigen = Nothing
        Dim oSapMgr As New ProcesoSAPManager(oAppContext, oAppSAPConfig)
        oAlmacenOrigen = oCatalogoAlmacenMgr.Load(oSapMgr.Read_Centros())

        ebSucOrigenCod.Text = oAlmacenOrigen.ID
        ebSucOrigenDesc.Text = oAlmacenOrigen.Descripcion
    ...
    Private Function Fm_bolTxtValidar() As Boolean
        ...
        If (oAlmacenDestino Is Nothing) Then
            ...
            bRes = False
        ElseIf (oAlmacenDestino.ID = oAppContext.ApplicationConfiguration.Almacen) Then

            MsgBox("El Almácen Destino no es valido", MsgBoxStyle.Exclamation, "DPTienda")
            ebSucDestinoCod.Focus()
            bRes = False

        ElseIf (ebSucOrigenCod.Text = String.Empty) Or (ebResponsable.Text = String.Empty) Then
            MsgBox("Para Guardar un Traspaso vaya a la Opción Nuevo.", MsgBoxStyle.Exclamation, "DPTienda")
            bRes = False
        ElseIf (ebSucOrigenCod.Text = oAlmacenDestino.ID) Then
        ...
        ElseIf (ebNumBulto.Text <= 0) Then
            ...
            ebNumBulto.Focus()
            bRes = False
        ElseIf (ebNumGuia.Text.Trim = String.Empty) Then
        ...
        ElseIf (ebObservaciones.Text.Trim = String.Empty) Then
            ...
            bRes = False
        ElseIf (Me.ebFolioObs.Text.Trim = String.Empty And Me.ebFolioObs.Visible) Then
        ...
        Else
            ...
            If dtTraspasoCorrida Is Nothing OrElse dtTraspasoCorrida.Rows.Count = 0 Then
                MsgBox("El Detalle del Traspaso no cuenta con Registros.", MsgBoxStyle.Exclamation, "DPTienda")
                bRes = False
                'Si es mayor a cero validamos que el total de piezas del traslado no supere el limite de piezas permitidas
            ElseIf oConfigCierreFI.PzasTotalesTS > 0 AndAlso odtArticulosTras.Compute("SUM(Cantidad)", "") > oConfigCierreFI.PzasTotalesTS Then
                MessageBox.Show("Las piezas del traspaso superan el límite permitido.", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                bRes = False
            Else
    ...
    Private Sub Sm_GuardarTraspaso(Optional ByVal bolImprimir As Boolean = False)
        ...
        If (oTraspasoSalida Is Nothing) Then   'Opción Guardar.
            ...
            For Each row As DataRow In dtMat.Rows
                ...
                row.AcceptChanges()
            Next
            If ebObservaciones.SelectedValue = "13" Then
                EdBxFolioSAP.Text = oSap.pedido_trasladomm02(dtMat, "1000", "M001", hGuia)
            Else
                EdBxFolioSAP.Text = oSap.pedido_trasladomm02(dtMat, oTraspasoSalidaMgr.centro_destino(oAlmacenDestino.ID.Remove(0, 1)), "M001", hGuia)
            End If
            ...
            Else
                ...
                'If ebObservaciones.SelectedValue = "13" Then
                '    strNumTras = oSap.trasladomm02(EdBxFolioSAP.Text, Me.ebObservaciones.Text & " " & ebFolioObs.Text, "000")
                'Else

                '    strNumTras = oSap.trasladomm02(EdBxFolioSAP.Text, Me.ebObservaciones.Text & " " & ebFolioObs.Text, Me.ebSucDestinoCod.Text.Trim)
                'End If
                ...
                'End If
            End If
            Dim RowSerie() As DataRow = dtMat.Select("Serie<>''")
            If RowSerie.Length > 0 Then
                Dim response As New Dictionary(Of String, Object)
                response = oSap.ZMMOC03(CStr(hGuia("NUMDOCMAT")))
            ...
            If ebObservaciones.SelectedValue = "13" Then
                oTraspasoSalida.AlmacenDestinoID = "000"
            Else
            ...
            oTraspasoSalida.Modulo = "ET" 'Entre Tiendas
            ...
            '-------------------------------------------------------------------------------------------------------------------------------
            'Aplicamos el traspaso y sacamos existencia del inventario
            '-------------------------------------------------------------------------------------------------------------------------------
            Sm_AplicarTraspaso()
            '-------------------------------------------------------------------------------------------------------------------------------
            'Enviamos correo a los auditores en caso que se sobrepase el limite de traspasos de salida permitidos
            '-------------------------------------------------------------------------------------------------------------------------------
            If oConfigCierreFI.TraspasosSalidaXDia > 0 Then EnviarCorreoLimiteTraspasosDelDia()
            '-------------------------------------------------------------------------------------------------------------------------------
            'Imprimir Traspaso
            '-------------------------------------------------------------------------------------------------------------------------------
    ...
    Private Function GetTraspasosDelDia() As Integer

        Dim dtTemp As DataTable
        'Dim oRow As DataRow
        'Dim Cant As Integer = 0
        'Dim Traspasos As String = ""

        dtTemp = oTraspasoSalidaMgr.SelectTraspasosByDate(Today, "ET").Copy 'ET = Entre Tiendas

        'For Each oRow In dtTemp.Rows
        '    If InStr(Traspasos.Trim.ToUpper, CStr(oRow!Folio).Trim.ToUpper) <= 0 Then
        '        Traspasos &= CStr(oRow!Folio).Trim.ToUpper & ","
        '        Cant += 1
        '    End If
        'Next
    ...
    Private Sub EnviarCorreoLimiteTraspasosDelDia()

        Try
            '---------------------------------------------------------------------------------------------------------------------------------
            'Verificamos si los traspasos realizados sobrepasan el limite de traspasos permitido por dia y enviamos un correo en este caso
            'a los auditores, a los de logistica y al gerente de plaza
            '---------------------------------------------------------------------------------------------------------------------------------
1:          Dim Cant As Integer = 0
2:          Cant = GetTraspasosDelDia()
3:          If Cant > oConfigCierreFI.TraspasosSalidaXDia Then
4:              Dim mmMail As New MailMessage
5:              Dim objSmtpServer As SmtpMail
6:              Dim strTo As String = ""
7:              Dim strMail As String = ""
                '------------------------------------------------------------------------------------------------------------------------------
                'Agregamos los correos de los auditores
                '------------------------------------------------------------------------------------------------------------------------------
8:              For Each strMail In oConfigCierreFI.CuentasCorreoAuditoria
9:                  If Not strMail Is Nothing AndAlso strMail.Trim <> "" Then
10:                     strTo &= strMail.Trim & ";"
                    End If
                Next
                '------------------------------------------------------------------------------------------------------------------------------
                'Agregamos los correos de Logistica de Tiendas
                '------------------------------------------------------------------------------------------------------------------------------
11:             For Each strMail In oConfigCierreFI.CuentasCorreoLogistica
12:                 If Not strMail Is Nothing AndAlso strMail.Trim <> "" Then
13:                     strTo &= strMail.Trim & ";"
                    End If
                Next
                '------------------------------------------------------------------------------------------------------------------------------
                'Agregamos los correos del gerente de plaza y del gerente de operaciones si están configurados
                '------------------------------------------------------------------------------------------------------------------------------
                If oConfigCierreFI.MailGerentePlaza.Trim <> "" Then strTo &= oConfigCierreFI.MailGerentePlaza & ";"
                If oConfigCierreFI.MailGerenteOperaciones.Trim <> "" Then strTo &= oConfigCierreFI.MailGerenteOperaciones & ";"
                '------------------------------------------------------------------------------------------------------------------------------
                'Construimos el cuerpo del Mail
                '------------------------------------------------------------------------------------------------------------------------------
15:             mmMail.From = oConfigCierreFI.CuentaCorreo.Trim
16:             mmMail.To = strTo
17:             mmMail.Subject = "Se ha sobrepasado el límite de traspasos permitidos"
18:             mmMail.Body = "Se ha sobrepasado el límite de traspasos permitido por día en la sucursal " & _
                          oAppContext.ApplicationConfiguration.Almacen & vbCrLf & vbCrLf & "Limite: " & _
                          oConfigCierreFI.TraspasosSalidaXDia & vbCrLf & "Traspasos Realizados: " & Cant
                'Dim oAttachment As MailAttachment = New MailAttachment(strruta)
                'mmMail.Attachments.Add(oAttachment)
19:             objSmtpServer.SmtpServer = oConfigCierreFI.ServidorSMTP
                Try
20:                 objSmtpServer.Send(mmMail)
                ...
                Catch ex As Exception
                    ...
                    EscribeLog(ex.ToString, "Error al enviar el correo de límite de traspasos de salida")
                End Try

            End If
        Catch ex As Exception
            EscribeLog(ex.ToString, "Error al generar el correo de limite de traspasos de salida. Linea " & Err.Erl)
        End Try
    ...
    Private Sub Sm_CapturaDetalle()
        ...
        Dim ofrmCapturaTraspasos As New frmInvTraspasosDSalidaCaptura(ebSucDestinoCod.Text)
...
#Region "Métodos Privados [Eventos]"
    ...
    Private Sub frmInvTraspasosDSalida_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If oConfigCierreFI.PedidoCompra = False Then
            GridTraspasoCorrida.RootTable.Columns("Serie").Visible = False
        End If
    ...
    Public Sub PrintComprobanteTraspaso()
        ...
        If Not oTraspasoSalida Is Nothing Then
            ...
            Dim oARReporte As New rptReporteTraspasoDeSalida(oTraspasoSalida, "Entre Tiendas")
...
BuscaAlmacen:
            ...
            Else
                ...
                If (oAlmacenDestino.ID = oAppContext.ApplicationConfiguration.Almacen) OrElse oAlmacenDestino.Descripcion.Trim.ToUpper = "TODOS" Then
                ...
                ElseIf oAlmacenDestino.ID = "000" Then
                    ebSucDestinoCod.Text = ""
                    ebSucDestinoDesc.Text = ""
                    MessageBox.Show("No se Pueden Hacer traspasos de Tiendas al Centro de Distribución", "DPTienda", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    oAlmacenDestino = Nothing
                    e.Cancel = True
                    Exit Sub
                ElseIf oTraspasoSalidaMgr.centro_destino(oAlmacenDestino.ID.Remove(0, 1)).Trim.Substring(0, 1) = "Z" Then
                    Me.ebSucDestinoCod.Text = "000"
                    GoTo BuscaAlmacen
                    '----------------------------------------------------------------------------------------------------------------
                    'Si la opción esta activada no permitimos que continue si intentan enviar a una tienda de otra plaza
                    '----------------------------------------------------------------------------------------------------------------
                ElseIf oConfigCierreFI.SoloMismaPlazaTS AndAlso oAlmacenDestino.PlazaID.Trim.ToUpper <> oAlmacenOrigen.PlazaID.Trim.ToUpper Then
                    ebSucDestinoCod.Text = ""
                    ebSucDestinoDesc.Text = ""
                    MessageBox.Show("No es posible realizar un traspaso a una tienda de otra plaza mediante esta opción.", "DPTienda", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    oAlmacenDestino = Nothing
                    e.Cancel = True
                    Exit Sub
                End If
