...
Public Class frmInvTraspasosDSalidaCaptura
...
#Region " Código generado por el Diseñador de Windows Forms "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(frmInvTraspasosDSalidaCaptura))
        ...
        Me.Name = "frmInvTraspasosDSalidaCaptura"
...
#Region "Variables Miembros"
    ...
    '-----------------------------------------------------------------------------------------------------------
    'FLIZARRAGA 19/02/2018: Se agrego para validar si hay artículo de consignación
    '-----------------------------------------------------------------------------------------------------------

    Private oTraspasoEntradaMgr As TraspasosEntradaManager
    Private Serie As String = ""
    Private HasSerie As Boolean = False
...
#Region "Métodos Privados"
    ...
    Private Sub Sm_Inicializar()
        ...
        If oConfigCierreFI.PedidoCompra = False Then
            grTraspasoCorrida.RootTable.Columns("Serie").Visible = False
        End If
    ...
    Private Sub Sm_BuscarArticulo(Optional ByVal Vpa_bolOpenRecordDialog As Boolean = False)
        ...
        If (Vpa_bolOpenRecordDialog = True) Then
            ...
            Try
                ...
                '---------------------------------------------------------------------------------------
                ' JNAVA (10.03.2016): Cargamos el codigo en el Txt box correspondiente
                '---------------------------------------------------------------------------------------
                Me.ebCodigo.Text = cArticulo
                '---------------------------------------------------------------------------------------
    ...
    Private Sub Sm_GenerarArticuloCorrida()
        ...
        If Not boolManual And Not dtCorridaArticulo Is Nothing Then
            '------------------------------------------------------------------------------------------------------------
            ' JNAVA (10.03.2016): Se modifico par que se filtrara en base a las existencias y no a la talla
            '------------------------------------------------------------------------------------------------------------
            'Dim odataview As New DataView(dtCorridaArticulo, "Talla = '" & strNumero & "'", "Talla", DataViewRowState.CurrentRows)
            '------------------------------------------------------------------------------------------------------------
            Dim odataview As New DataView(dtCorridaArticulo, "Existencia > 0 ", "Talla", DataViewRowState.CurrentRows)
            If odataview.Count > 0 Then
                '------------------------------------------------------------------------------------------------------------
                ' JNAVA (10.03.2016): Seteamos la talla
                '------------------------------------------------------------------------------------------------------------
                strNumero = odataview(0)("Talla")
                '------------------------------------------------------------------------------------------------------------
            ...
            Else
                ...
                'dtCorridaArticulo = Nothing
                'Sm_TxtLimpiar()
                MsgBox("No hay existencia en Talla " & strNumero, MsgBoxStyle.Exclamation, Me.Text)
                'Me.ebCodigo.Focus()
            End If
            'Sm_TxtLimpiar()
            'boolManual = False
    ...
    Private Function Fm_bolTxtValidar() As Boolean
        ...
        Dim dtZequi As DataTable = oTraspasoEntradaMgr.GetZequiByOption(1, oArticulo.CodArticulo)
        Dim Zequi As Zequi = Nothing
        Serie = ""
        HasSerie = False
        If dtZequi.Rows.Count > 0 Then
            Serie = InputBox("Inserte el Número de Serie", "Dportenis PRO")
            If Serie.Trim() <> "" Then
                Zequi = oTraspasoEntradaMgr.GetZequi(CStr(dtZequi.Rows(0)!CodArticulo), Serie)
                If Zequi.CodArticulo.Trim() <> "" OrElse Zequi.Serie.Trim() <> "" Then
                    HasSerie = True
                Else
                    Serie = ""
                    MessageBox.Show("El número de serie no corresponde al código artículo", "Dportenis PRO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    Exit Function
                End If
            Else
                MessageBox.Show("Es necesario agregar el número de serie", "Dportenis PRO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Exit Function
            End If
        End If
        ...
        For Each dr In dtCorridaArticulo.Rows
            ...
            If Not (IsDBNull(dr!Cantidad) = True) Then
                ...
                drDataRow = dtTraspasoDetalle.Select("CodArticulo ='" & oArticulo.CodArticulo & "' AND Serie='" & Serie & "'") ' and Talla ='" & dr!Talla & "'")
        ...
        If bEntro = False Then
            MessageBox.Show("Es necesario especificar la cantidad en al menos una talla.", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Me.grArticuloCorrida.Focus()
            Exit Function
        End If
    ...
    '---------------------------------------------------------------------------
    ' JNAVA (11.04.2016): Se agrega el codigo
    '---------------------------------------------------------------------------
    Public Sub AgregarCodigo()
        ...
        For Each dr In dtCorridaArticulo.Rows
            ...
            If Not (IsDBNull(dr!Cantidad) = True) Then
                ...
                If (dr!Cantidad > 0) Then
                    ...
                    oTraspasoSalidaMgr.AgregarArticulo(oArticulo, dr!Talla, dr!Cantidad, (oArticulo.CostoProm * dr!Cantidad), intTraspasoID, Serie)
...
#Region "Métodos Privados [Eventos]"
    ...
    Private Sub frmInvTraspasosDSalidaCaptura_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        ...
        'Captura Manual
        'ALLOW
        If Me.ActiveControl Is Me.ebCodigo AndAlso Not e.KeyCode = Keys.Enter AndAlso Me.ebCodigo.ReadOnly = True Then
            ...
            'ALLOW
        ElseIf Me.ActiveControl Is Me.ebCodigo AndAlso Me.ebCodigo.ReadOnly = True AndAlso Me.ebCodigo.Text = "" AndAlso str.Trim <> "" Then
            str = str.Trim
            str = str.Replace("", "")
    ...
    Private Sub ebCodigo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles ebCodigo.KeyDown
        ...
        If e.Alt And e.KeyCode = Keys.S Then
            '---------------------------------------------------------------------------------------------------------------
            ' JNAVA (10.03.2016): Se valida si tiene permisos para captura manual del material ya que antes no lo hacia
            '---------------------------------------------------------------------------------------------------------------
        ...
        If boolManual AndAlso e.KeyCode = Keys.Enter Then
            SendKeys.Send("{TAB}")
        End If
    ...
    Private Function LoadCodigo(ByVal strCodigo As String) As String
        ...
        If oUPCMgr.IsSkuOrMaterial(strCodigo).Trim() = "S" Then
            'Buscamos Codigo UPC
            ...
            Else
                MsgBox("Código UPC No Existe. ", MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly, Me.Text)
...
#End Region
    ...
    Private Sub ModificarCantidadRowTC(ByVal Cantidad As Integer)
        ...
        End If
        '-----------------------------------------------------------------------------------------------------------------------------------
        'Validamos que no sobrepase las existencias
        '-----------------------------------------------------------------------------------------------------------------------------------
        oCurrentRow = grTraspasoCorrida.GetRow()

        dtExistencias = oTraspasoSalidaMgr.GenerarArticuloCorrida(oCurrentRow.DataRow!CodArticulo, vAlmacenDestino).Tables("Existencias")

        For Each oRow In dtExistencias.Rows
            If CStr(oRow!Talla).Trim.ToUpper = CStr(oCurrentRow.DataRow!Talla).Trim.ToUpper Then
                If oRow!Existencia < Cantidad Then
                    MessageBox.Show("La cantidad especificada supera el máximo de piezas en existencia.", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    GoTo Final
        ...
        With odrDataRow
            'Borramos los registros guardados
            oTraspasoSalidaMgr.EliminarArticulo(.Item("CodArticulo"), .Item("Talla"))
            'oTraspasoSalidaMgr.ActualizarLecturado(.Item("CodArticulo"), .Item("Talla"), 0, Cantidad)
            'Cargamos el objeto oArticulo
            oArticulo = oCatalogoArticulosMgr.Create
            oArticulo = oCatalogoArticulosMgr.Load(.Item("CodArticulo"))
            'Insertamos de nuevo el registro con la cantidad modificada
            oTraspasoSalidaMgr.AgregarArticulo(oArticulo, .Item("Talla"), Cantidad, (oArticulo.CostoProm * Cantidad), intTraspasoID)
            oArticulo = Nothing
        End With
Final:
        oCurrentRow = Nothing
    ...
    'Private Sub grTraspasoCorrida_CellValueChanged(ByVal sender As Object, ByVal e As Janus.Windows.GridEX.ColumnActionEventArgs) Handles grTraspasoCorrida.CellValueChanged

    '    If e.Column.Key.Trim.ToUpper = "SCantidad".Trim.ToUpper Then
    '        Dim strValor As String = Me.grTraspasoCorrida.GetValue("SCantidad")
    '        If strValor.Trim <> "" Then
    '            Dim Cant As Integer = CInt(strValor)
    '            If Cant < 1 Then
    '                Me.grTraspasoCorrida.SetValue("SCantidad", 1)
    '                Cant = 1
    '            End If
    '            ModificarCantidadRowTC(Cant)
    '        End If
    '    End If

    'End Sub

    Private Sub grTraspasoCorrida_CellUpdated(ByVal sender As Object, ByVal e As Janus.Windows.GridEX.ColumnActionEventArgs) Handles grTraspasoCorrida.CellUpdated
        If e.Column.Key.Trim.ToUpper = "SCantidad".Trim.ToUpper Then
            Dim strValor As String = Me.grTraspasoCorrida.GetValue("SCantidad")
            If IsDBNull(strValor) = False AndAlso strValor.Trim <> "" Then
                Dim Cant As Integer = CInt(strValor)
                If Cant < 1 Then
                    Me.grTraspasoCorrida.SetValue("SCantidad", 1)
                    Cant = 1
...
#Region "Adecuaciones Retail"
    ...
    Private Function ValidaExistenciaCorrida(ByVal Existencia As DataTable, ByVal Corrida As DataTable) As DataTable
